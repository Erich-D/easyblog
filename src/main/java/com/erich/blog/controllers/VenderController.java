...
public class VenderController {
    ...
    @GetMapping("/addVenderPost")
    public String addVenderPost(Model model, @RequestParam("blogPostId") Optional<String> blogPostId) {		
	setDefaultBlogPost(model);					
	return "vender/addVenderPost";
    }
	

    @PostMapping("/addVenderPost")
    public String addVenderPostSubmit(Model model, Post blogPost, HttpServletRequest request) {
	...
	System.out.println("Body is " + blogPost.getBody());
        ...
        blogPost.getTags().add(tp.findByName("alert"));
        blogPost.setAuthor(up.getById(2));
        rp.save(blogPost);
